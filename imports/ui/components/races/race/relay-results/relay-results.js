...
Template.raceRelayResults.onRendered(function () {
...
Template.raceRelayResults.onCreated(function () {
...
Template.raceRelayResults.helpers({
  categorySelected() {
  ...
  resultEntries() {
    ...
    const firstCompetitor = ResultListEntriesRelayTeams.findOne(query, projection);
    if (firstCompetitor) {
      template.maxLapNumberCompleted.set(firstCompetitor.lapsCompleted);
      if (firstCompetitor.lapsCompleted > 0) {
        template.firstCompetitorTimestamp.set(firstCompetitor.latestPerformance);
      }
    }
      ...
      fields: [
          ...
          fn(value) {
            ...
            if (value) {
              for (let i = 0; i < value.length; i++) {
                text += value[i].performance-previousLap+';';
                previousLap = value[i].performance;
              }
            }
            return text;
          },
        ...
        */
        ...
        },
        {
          ...
          sortable: false,
          fn(value) {
            return getStandardDeviation(value);
          },
        },        
        {
...
function generatePdfDoc(template) {
  ...
  const tables = [];
  //tables.push({ text: `Results of ${raceName} (TEAMS) > ${checkpoint.deviceId}`, style: 'header' });
  for (let i = 0; i < categoryList.length; i += 1) {
    ...
    const projection = { sort: { lapsCompleted: -1, latestPerformance: 1 } };
    const entries = ResultListEntriesRelayTeams.find(query, projection).map(function(item, index) {
      const element = item;
      element.calculatedRank = index + 1;
      return element;
    });
    tables.push({ text: `Team Relay Results ${raceName} > ${checkpoint.deviceId} - ${categoryList[i].identifier}`, style: 'header' });
    tables.push(`${entries.length} teams listed under this category`);
    const tableRows = [[
      { text: '#', alignment: 'right' },
      { text: 'Abbr', alignment: 'left' },
      'Team', 'Bibs', 'Laps', 'Splits',
      //{ text: 'Perform.', alignment: 'right' },
      { text: 'Best', alignment: 'right' },
    ]];
    for (let j = 0; j < entries.length; j += 1) {
      const performance = getPerformanceText(entries[i].latestPerformance, entries[i].lapsCompleted, maxLapNumberCompleted);
      const gap = getPerformanceGapText(entries[i].latestPerformance, entries[i].lapsCompleted, maxLapNumberCompleted, template.firstCompetitorTimestamp.get());
      let splits = '';
      for (let k = 0; k < entries[j].splits.length; k += 1) {
        splits += `${getPerformanceText(entries[j].splits[k].performance, 1, 1)} `;
      }
      const bestLap = getBestLap(entries[j].splits);
      tableRows.push([{ text: entries[j].calculatedRank, color: 'gray', alignment: 'right' },
        { text: entries[j].teamAlternate, color: 'gray', alignment: 'right' },
        { text: entries[j].teamName, italics: true, color: 'gray' },
        { text: entries[j].bibs, italics: true, color: 'gray' },
        { text: entries[j].lapsCompleted, italics: true, color: 'gray'},
        { text: splits, color: 'gray', alignment: 'right' },
        //{ text: performance, color: 'gray', alignment: 'right' },
        { text: bestLap, color: 'gray', alignment: 'right' },
      ]);
    }
    tables.push({
      style: 'tableResults',
      table: {
        widths: [15, 35, 100, 30, 20, 450, 40],
        body: tableRows,
      },
    });
  }
...
Template.raceRelayResults.events({
